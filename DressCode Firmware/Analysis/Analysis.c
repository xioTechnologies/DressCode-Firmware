...
#define SR 16129.03224f 	// moved to 16 mips
...
typedef struct Goertzel {
	...
	unsigned int i; // index in the current block
	_Q16 q0, q1, q2;
	_Q16 c;
	_Q16 magnitude;
	_Q16 magScaler;
...
static int  SF_update(SpectralFluxOnsetDetector *sf, _Q16 x);
static int  SF_readyP(SpectralFluxOnsetDetector *sf);
static _Q16 SF_getFlux(SpectralFluxOnsetDetector *sf);
static int  SF_getOnset(SpectralFluxOnsetDetector *sf);
